...
while read LINE; do
	...
	r2 -Q -c "aaa; s main; e asm.arch > $TMPFOLDER/arch.txt; agj > $TMPFOLDER/agj.json; isj > $TMPFOLDER/isj.json; izj > $TMPFOLDER/izj.json; #!pipe r2dec > $TESTFOLDER/$NAME.output.txt" "$LINE" 2> $TMPFOLDER/stderr.txt
